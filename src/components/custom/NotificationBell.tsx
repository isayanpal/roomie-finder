"use client";
import { Unread } from "@/constants/interfaces";
        ...
        (payload: any) => {
          ...
          if (msg.receiverId === userId) {
            setUnread((prev) => {
              ...
              if (ex)
                return prev.map((u) =>
                  u.senderId === msg.senderId ? { ...u, count: u.count + 1 } : u
                );
              return [
                ...prev,
                { senderId: msg.senderId, count: 1, senderName: "..." },
              ];
            });
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-64 bg-white shadow rounded z-50">
          ...
          {unread.length === 0 && (
            <div className="p-2 text-gray-500">No unread</div>
          )}
          {unread.map((u) => (
            <button
              key={u.senderId}
